...
export function AppSidebar() {
  ...
  return (
    <aside
      className={cn(
        'fixed left-0 top-0 z-50 flex h-screen w-64 flex-col border-r border-white/10 bg-zinc-900/95 backdrop-blur-md transition-transform duration-300 ease-in-out',
        isSidebarOpen ? 'translate-x-0 shadow-[20px_0_30px_rgba(0,0,0,0.5)]' : '-translate-x-[calc(100%-12px)]'
      )}
    ...
    >
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-white/5 px-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <span className="truncate text-base font-bold text-white">NossoCRM</span>
        </div>
      ...
      >
        {BLOCKS.map((block) => (
          <div key={block.title} className="px-2">
            <p className="mb-2 mt-4 px-3 text-[10px] font-bold uppercase tracking-wider text-zinc-500 first:mt-0">
              {block.title}
            </p>
      ...
      <div className="mt-auto shrink-0 border-t border-white/5 pt-4">
        <div className="flex flex-wrap gap-2 px-3 pb-2">
          <button
            type="button"
            className="flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs text-zinc-500 transition-colors hover:bg-white/5 hover:text-zinc-300"
          >
        ...
        <div className="relative px-3 pb-4">
          <button
            ...
            className="flex w-full cursor-pointer items-center gap-3 rounded-xl border border-white/5 bg-white/5 p-3 transition-colors hover:bg-white/10 focus-visible:outline focus-visible:ring-2 focus-visible:ring-green-500/50"
          >
            {profile?.avatar_url ? (
              <Image
              ...
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-600 text-sm font-bold text-white ring-2 ring-white/10" aria-hidden>
                {initials}
              </div>
            )}
          ...
          {isUserMenuOpen && (
            <>
              ...
              <div className="absolute bottom-full left-3 right-3 z-50 mb-2 overflow-hidden rounded-xl border border-white/10 bg-zinc-900 shadow-xl backdrop-blur-md">
                <div className="p-1">
                  <Link
                    href="/profile"
